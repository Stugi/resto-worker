<template>
    ...
    >
        <!-- Modal Content -->
        <div
            class="bg-bg-card rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col"
        >
            <!-- Header -->
            <div class="px-6 py-4 border-b border-border shrink-0">
                <h2 class="text-xl font-semibold text-text">Оплата подписки</h2>
            ...
            <div class="p-6 overflow-y-auto flex-1">
                        ...
                        <div class="space-y-1 text-sm text-text-secondary">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-text"
                                    >до {{ tariff.maxRestaurants }}</span
                                >
                            </div>
                            <div class="flex justify-between">
                                <span>Пользователи:</span>
                                <span class="font-medium text-text"
                                    >до {{ tariff.maxUsers }}</span
                                >
                            </div>
                            <div class="flex justify-between">
                                <span>Транскрипций:</span>
                                <span class="font-medium text-text"
                                    >{{ tariff.maxTranscriptions }}/мес</span
                                >
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <p class="text-sm">
                        Ссылка на оплату отправлена в Telegram
                    </p>
                </div>
            ...
            <!-- Footer -->
            <div
                class="px-6 py-4 border-t border-border flex gap-3 justify-end shrink-0"
            >
                <BaseButton
                    @click="$emit('close')"
                    variant="secondary"
...
const handlePayment = async () => {
    ...
    try {
        ...
        error.value =
            err.data?.message || err.message || "Ошибка создания платежа";
    } finally {
        loading.value = false;
    }
};
...
const handleSendToTelegram = async () => {
    ...
    try {
        ...
        error.value =
            err.data?.message || err.message || "Ошибка отправки в Telegram";
    } finally {
