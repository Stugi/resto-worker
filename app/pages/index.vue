<template>
    <div>
        ...
        >
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-text mb-1">
                    Аналитика
                </h1>
                <p class="text-text-secondary text-sm">
                    Классификация отзывов и тренды
                </p>
            </div>
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <BaseSelect
                    ...
                    :searchable="false"
                    class="w-full sm:w-48 sm:max-w-48"
                />
        ...
        <div
            v-else-if="error"
            class="bg-status-red-bg border border-status-red-border rounded-lg p-5 text-center"
        >
        ...
        <template v-else-if="data">
            ...
            >
                <svg
                    class="w-5 h-5 text-status-orange-icon shrink-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
                ...
                <p class="text-sm text-text">
                    <strong>{{ data.totalUnclassified }}</strong> транскрипций
                    ещё не классифицированы. Они будут классифицированы при
                    следующем автоматическом формировании отчёта.
                </p>
            </div>
        </template>
    </div>
...
const fetchData = async () => {
    ...
    try {
        ...
        data.value = await $fetch<AnalyticsResponse>(
